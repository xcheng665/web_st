...
def init_session_state():
    ...
    defaults = {
        ...
        'df': None  # 将DataFrame存储在session_state中
    }
...
def import_from_csv(file):
    ...
    try:
        ...
        if all(col in df.columns for col in required_columns):
            ...
            st.session_state.df = df[['id', '规范', '条文', '条文内容']]
            return True
        else:
...
def import_ontology(file):
    """导入本体文件（支持RDF/XML, TTL, N3等格式）"""
    try:
        # 这里简化处理，实际应用中需要使用rdflib等库来解析本体
        content = file.getvalue().decode("utf-8")
        
        # 模拟解析本体内容并转换为条文数据
        # 在实际应用中，这里需要使用适当的本体解析库
        st.info("本体文件已上传，正在解析...")
...
def show_import_modal():
    ...
    with st.container():
        ...
        if import_option == "CSV文件":
            ...
            st.info("支持RDF/XML, Turtle(.ttl), N3, OWL等本体格式")
...
def get_dataframe() -> pd.DataFrame:
    ...
    if st.session_state.df is None:
        file_path = r"E:\Users\czy\Desktop\web_st\规范数据集\GB 50016-2014(2018年版) 建筑设计防火规范.csv"
